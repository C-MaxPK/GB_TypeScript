export interface Database {
  ...
  bookedDates: [],
  price?: number,
  totalPrice?: number // появляется в _formatFlatObject()
}
...
export class FlatRentSdk {
  ...
  get(id: string): Promise<Database | null | Error> // Error?
  search(parameters: Parameters): Promise<Database[] | Error> // Error?
  book(flatId: number, checkInDate: Date, checkOutDate: Date): Promise<number | Error> // Error?
